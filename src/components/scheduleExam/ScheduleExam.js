...
import { faCalendarAlt, faCheck, faTimes, faClock, faStream, faBook } from '@fortawesome/free-solid-svg-icons';
import {
    account, databases, database_id, examsTable_id, ID
...
const ScheduleExam = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const selectedSubjectObj = subjects.find(sub => sub.examTableId === selectedSubject);
        ...
        try {
            ...
            try {
                ...
                const saveResponse = await databases.createDocument(database_id, examsTable_id, ID.unique(), examSetData);
                console.log('exam saved successfully', saveResponse);
            } catch (e) {
                console.log('Failed to save Exam Set Data to Appwrite Database Collection: ', e);
            }
        } catch (error) {
                        ...
                        <Card.Body>
                            ...
                            <Form onSubmit={handleSubmit}>
                                <Form.Group controlId="classSelect">
                                    <Form.Label><FontAwesomeIcon icon={faStream} /> Select Class</Form.Label>
                                ...
                                {streams.length > 0 && (
                                    <Form.Group controlId="streamSelect">
                                        <Form.Label><FontAwesomeIcon icon={faStream} /> Select Stream</Form.Label>
                                ...
                                <Form.Group controlId="subjectSelect">
                                    <Form.Label><FontAwesomeIcon icon={faBook} /> Select Subject</Form.Label>
                                    <Form.Control as="select" value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)}>
                                        <option value="">Select Subject</option>
                                        {subjects.map(sub => (
                                            <option key={sub.id} value={sub.examTableId}>{sub.subjectName}</option>
                                        ))}
                                    </Form.Control>
                                </Form.Group>

                                <Form.Group controlId="startDate">
                                    <Form.Label><FontAwesomeIcon icon={faCalendarAlt} /> Select Start Date and Time</Form.Label>
                                    <DatePicker
                                        selected={startDate}
                                ...
                                <Form.Group controlId="endDate">
                                    <Form.Label><FontAwesomeIcon icon={faCalendarAlt} /> Select End Date and Time</Form.Label>
                                    <DatePicker
                                        selected={endDate}
                                ...
                                <Form.Group controlId="duration">
                                    <Form.Label><FontAwesomeIcon icon={faClock} /> Set Duration (in minutes)</Form.Label>
                                    <Form.Control
                                        type="number"
                                        value={duration}
