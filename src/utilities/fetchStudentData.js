...
const updateExamData = async (examData) => {
  try {
    ...
    await db.transaction("rw", db.exams, async () => {
      ...
      const savingToIndexDB = await db.exams.bulkPut(
        examData.map((exam) => ({
          ...exam,
          examID: exam.examID,
...
export const updateSubjectsData = async () => {
  try {
    ...
    await db.transaction("rw", db.subjects, async () => {
      ...
      // Bulk put the new data after clearing the table
      const savingToIndexDB = await db.subjects.bulkPut(
...
export const updateResultsData = async (studID) => {
  try {
    // Fetch Results
    const results = await getDocs(studentMarksTable_id, [
    ...
    await db.transaction("rw", db.results, async () => {
      ...
      // Bulk put the new data after clearing the table
      const savingToIndexDB = await db.results.bulkPut(
        results.map((result) => ({
          // ...result,
          ...
          durationMINS:
            result.durationMINS != undefined || result.durationMINS != null
              ? result.durationMINS
              : null,
...
export const updateClassData = async () => {
  try {
    ...
    await db.transaction("rw", db.classes, async () => {
      ...
      // console.log('Saving to Subjects IndexDB ... ');
      // Bulk put the new data after clearing the table
      const savingToIndexDB = await db.classes.bulkPut(
        classes.map((class_) => ({
          ...class_,
...
export const updateSchoolData = async () => {
  try {
    ...
    await db.transaction("rw", db.schoolData, async () => {
      ...
      // Bulk put the new data after clearing the table
      const savingToIndexDB = await db.schoolData.bulkPut(
        schoolData.map((data) => ({
          ...data,
 ...
 */
export const fetchStudents = async (refresh = true) => {
  try {
    ...
    if (refresh) {
      ...
      if (response.ok) {
        ...
        console.warn(
          "Failed to fetch students data from local file, trying the database refresh."
        );
...
export const fetchSetExams = async (userInfo) => {
  try {
    ...
    if (userInfo.userType === "student") {
      ...
      throw new Error(
        "Invalid user type or user type is not provided or supported: ",
        userInfo.userType
      );
    }
